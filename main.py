...
# Tenta importar a função main do arquivo auto_200.py. Se falhar, define uma função mock para testes
try:
    from auto_200 import main as auto_200_main

except ImportError:
    def auto_200_main(stop_event=None):
        print("ERRO: O arquivo 'auto_200.py' não foi encontrado ou está com erro.")
        time.sleep(5)
...
class ScheduleWindow(ctk.CTkToplevel):
    def __init__(self, parent):
        ...
        self.title("Agendamento de Tarefas")
        self.geometry("500x400")
        self.resizable(False, False)
        
        self.transient(parent)  # Torna a janela filha da janela principal
    ...
    def setup_ui(self):
        ...
        title_label = ctk.CTkLabel(main_frame, text="Agendamento de Tarefas", font = ctk.CTkFont(size = 18, weight="bold")) # Ainda não sei se vai usar isso
        title_label.pack(pady=(10, 20))

        add_frame = ctk.CTkFrame(main_frame)
        add_frame.pack(fill="x", padx = 10, pady=(0, 20))

        add_label = ctk.CTkLabel(add_frame, text="Adicionar Novo Agendamento:")
        ...
        add_button = ctk.CTkButton(input_frame, text="Adicionar", command=self.add_schedule, width = 100)
        ...
        self.schedule_scoll = ctk.CTkScrollableFrame(list_frame, height = 150)
        self.schedule_scoll.pack(fill="both", expand=True, padx=10, pady=(0, 10))

        control_frame = ctk.CTkFrame(control_frame, text = "Fechar", command=self.destroy, width = 100)
        control_frame.pack(fill = "x", padx = 10)

        close_button = ctk.CTkButton(control_frame, text="Fechar", command=self.destroy, width = 100)
        close_button.pack(side = "right", padx = (10, 0))

        clear_button = ctk.CTkButton(control_frame, text="Limpar Tudo", command=self.clear_schedules, width = 100, hover_color = "darkred")
    ...
    def clear_all_schedules(self):
        ...
        dialog = ctk.CTkInputDialog(text="Digite 'CONFIRMAR' para remover todos os agendamentos:", title="Confirmação")
    ...
    def update_schedule_list(self):
        ...
        if not self.schedules:
            no_schedule_label = ctk.CTkLabel(self.schedule_scroll, text="Nenhum agendamento configurado", text_color="gray")
        ...
        for i, time_str in enumerate(self.schedules):
            ...
            time_label = ctk.CTkLabel(schedule_frame, text=f"🕐 {time_str}", font=ctk.CTkFont(size=14))
            time_label.pack(side="left", padx=10, pady=5)
            
            remove_button = ctk.CTkButton(schedule_frame, text="Remover", command=lambda t=time_str: self.remove_schedule(t), width=80, height=25, fg_color="darkred", hover_color="red")
            remove_button.pack(side="right", padx=10, pady=5)
    
    def show_error(self, message):
        error_dialog = ctk.CTkInputDialog(text=f"ERRO: {message}\n\nPressione OK para continuar", title="Erro")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        title_label = ctk.CTkLabel(main_frame, text="Automação 200 - SSW", font=ctk.CTkFont(size=24, weight="bold"))
        ...
        self.start_button = ctk.CTkButton(action_frame, text="Iniciar Automação", command=self.start_automation)
        self.start_button.grid(row=0, column=0, padx=5)

        self.stop_button = ctk.CTkButton(action_frame, text="Parar Automação", command=self.stop_automation, state="disabled")
        ...
        schedule_label = ctk.CTkLabel(schedule_frame, text="Sistema de Agendamento", font=ctk.CTkFont(size=16, weight="bold"))
...
if __name__ == "__main__":
    ...
    def setup_tray():
        ...
        icon = pystray.Icon("auto_200", image, "Automação 200", menu)
        icon.run()
